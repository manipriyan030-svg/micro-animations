...
export default function DownloadModal({ onClose }: DownloadModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl mx-4 rounded-2xl border border-card-border bg-surface p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {/* Code */}
        <div className="relative rounded-xl bg-surface-2 p-4 overflow-auto max-h-80 border border-border-subtle">
          <pre className="text-sm text-accent font-mono whitespace-pre-wrap break-words">{codeMap[tab]}</pre>
        </div>
        ...
        <div className="mt-4 flex gap-3">
          <button
            onClick={handleCopy}
            className="flex-1 rounded-xl border border-border-subtle py-2.5 text-sm font-medium transition-all hover:bg-surface-2"
          >
